...
Docker を使わず、手元の PC に Node.js と Python を用意して実行する場合の手順です。
...
ローカル環境を整える代わりに、Docker コンテナを用いてサーバーを起動することもできます。以下のコマンドはホスト OS で実行し、コンテナ内で自動的にビルドとサーバー起動が行われます。
...
以下のコマンドはローカル環境、またはコンテナ内で Python を実行できる状態で使用し
ます。
...
この節のコマンドもローカル環境での実行を想定しています。Docker を利用する場合は
`docker run --rm -it -v $(pwd):/app pointcloud-slide bash` などでコンテナに入り、同
じコマンドを実行してください。
...
以下の操作はローカル環境で実行します。Python と PyInstaller がインストールされた環
境が必要です。
